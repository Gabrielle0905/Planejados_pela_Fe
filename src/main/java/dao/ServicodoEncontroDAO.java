...
public class ServicodoEncontroDAO {
	...
	public ServicoDoEncontro buscarId(int idServico) {
		String sql = "SELECT *, encontro_id FROM ServicoDoEncontro WHERE idServico = ?";
		...
		try(Connection conn = ConnectionFactory.getConnection()){
			...
			if(rs.next()) {
				...
				int idMae = rs.getInt("mae_id");
	            Mae mae = new MaeDAO().buscarId(idMae); 
	...
	public List<ServicoDoEncontro> listartodos(){
		String sql = "SELECT * FROM ServicoDoEncontro";
